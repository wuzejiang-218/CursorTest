...
# 1) install.sh at kit root: ./install.sh + ./cli + ./plugin + ./cli/skills
# 2) install.sh inside cli folder: ./cli/install.sh + ./cli/plugin + ./cli/skills + cli files
if [[ -d "${SCRIPT_DIR}/cli" ]]; then
  CLI_SRC_DIR="${SCRIPT_DIR}/cli"
  PLUGIN_SRC_DIR="${SCRIPT_DIR}/plugin"
  SKILLS_SRC_DIR="${SCRIPT_DIR}/cli/skills"
else
...
install_skill() {
  local find_skill_src="${SKILLS_SRC_DIR}/find-skills/SKILL.md"
  local preference_skill_src="${SKILLS_SRC_DIR}/skillhub-preference/SKILL.md"
  local legacy_find_skill_src=""
  local legacy_preference_skill_src=""
  local installed=0

  if [[ -d "${SCRIPT_DIR}/cli" ]]; then
    legacy_find_skill_src="${SCRIPT_DIR}/skill/SKILL.md"
    legacy_preference_skill_src="${SCRIPT_DIR}/skill/SKILL.skillhub-preference.md"
  else
    legacy_find_skill_src="${SCRIPT_DIR}/skill/SKILL.md"
    legacy_preference_skill_src="${SCRIPT_DIR}/skill/SKILL.skillhub-preference.md"
  fi

  if [[ ! -f "${find_skill_src}" && -f "${legacy_find_skill_src}" ]]; then
    find_skill_src="${legacy_find_skill_src}"
  fi
  if [[ ! -f "${preference_skill_src}" && -f "${legacy_preference_skill_src}" ]]; then
    preference_skill_src="${legacy_preference_skill_src}"
  fi
